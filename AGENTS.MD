...
| `.tx/config` | Transifex CLI config for community translations |

**Migration pattern** (converting a hardcoded string):
```vue
<!-- Before -->
<h2>Übersicht</h2>
...
In `<script setup>` blocks use `const { t } = useI18n()` and call `t('key')`.
